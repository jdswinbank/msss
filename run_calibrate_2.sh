...
usage() {
    echo -e "Usage:"
    echo -e "    ${0} [options] <obs_id> <band> <skyModel> <calModel> \n"
    echo -e "Options with string arguments:"
    echo -e '    -o   Output filename (default: ${obs_id}_SAP000_BAND${band}.MS)'
...
# Beams are "always" 0/2 in this mode
targbeam=0
calbeam=1
...
if [ ! ${OUTPUT_NAME} ]; then
    OUTPUT_NAME=${obs_id}_SAP00${targbeam}_BAND${band}.MS
fi
...
declare -a SUBBAND_NUMBERS
declare -a CALBAND_NUMBERS
for num in {0..9}; do
    SUBBAND_NUMBERS[${num}]=`printf %03d $(($band*10+$num))`
    CALBAND_NUMBERS[${num}]=`printf %03d $(($band*10+122+$num))`
done
...
if [ ${COLLECT} = "TRUE" ]; then
    ...
    OLDIFS=${IFS}
    IFS=","
    BANDS="${SUBBAND_NUMBERS[*]},${CALBAND_NUMBERS[*]}"
    IFS=${OLDIFS}
    for node in {01..100} ; do
        echo locus$node
    done | xargs -n1 -P4 -Ihost scp -r host:/data/scratch/pipeline/${obs_id}*/*SB{$BANDS}_target_sub* . > log/collect.log 2>&1
    log "Data collected"
    trap failure ERR
fi
...
for num in {0..9}; do
    ...
    source=${obs_id}_SAP00${targbeam}_SB${SUBBAND_NUMBERS[${num}]}_target_sub.MS.dppp
    cal=${obs_id}_SAP00${calbeam}_SB${CALBAND_NUMBERS[${num}]}_target_sub.MS.dppp
